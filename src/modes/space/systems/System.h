...
struct SystemContext {
    ...
    // The same ContentLibrary `content` above points at, but non-const. Two writers:
    // EngineerSystem (architecture.md 12.12) registers a merged module so its id resolves
    // everywhere else `content.FindModule` already does; ConstructionSystem (architecture.md
    // 12.30.8) registers a saved Template's body so `content.FindShip` can resolve it back. Same
    // nullable-pointer shape as `economy`/`discovery`: most systems never touch it, and every
    // test fixture predating EngineerSystem constructs a SystemContext without one. A separate
    // pointer rather than making `content` itself non-const, so the "systems only read content"
    // invariant every other system relies on stays true in the type, not just by convention.
