...
//
// architecture.md 12.30.1 -- `PlayerLocation` below is the source of truth; this tag is
// *derived* from it, never emplaced directly by gameplay code.
// modes/space/systems/PlayerLocationSystem.h does the deriving, every tick: a registry with no
// PlayerLocation entity yet (OnEnter hasn't placed a player) leaves this component empty, so
// readers see an empty view rather than a stale tag, the safer of the two failure modes.
...
// root by PlayerLocationSystem; this is the only thing gameplay code ever writes.
